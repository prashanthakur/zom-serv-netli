...
app.get('/details/:id',(req,res) => {
    ...
    db.collection('restaurantsData').find({restaurant_id:Number(id)}).toArray((err,result)=>{
        if(err) throw err;
        res.send(result)
    })
})
...
app.get('/menu/:id',(req,res) => {
    var id = req.params.id
    console.log(id)
    db.collection('restaurantsmenu').find({restaurant_id:Number(id)}).toArray((err,result)=>{
        if(err) throw err;
        res.send(result)
    })
})
...
app.post('/menuItem',(req,res) => {
    ...
    db.collection('restaurantsmenu').find({menu_id:{$in:req.body.ids}}).toArray((err,result)=>{
        if(err) throw err;
        res.send(result)
    })
})
